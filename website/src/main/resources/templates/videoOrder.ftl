		...
		<script type="text/javascript">
			$(function(){
			    ...
			    $(".orderBtn").click(function(){
			     	var orderTimeId = $(this).data("timeid");
					$("#timeid").val(orderTimeId);
					$("#searchForm").ajaxSubmit(function (data) {
						if(data.success){
						    $.messager.confirm("提示","操作成功",function () {
								window.location.href="/videoAuditOrder?time=" + new Date().getTime();
                            });
						}else{
		...
		<div class="container">
			<div class="row">
				...
				<div class="col-sm-9">
                    ...
                    <p class="bg-success">预约成功：预约客服 [ ${videoAuditOrder.auditor.username} ] ,时间 ：[ ${videoAuditOrder.orderDate?string("yyyy-MM-dd")} -  ${videoAuditOrder.orderTime.begin}  -  ${videoAuditOrder.orderTime.end} ]
						<a>撤销预约</a>
					</p>

					<#else>
					<form action="/saveVideoAuditOrder" name="searchForm" id="searchForm" class="form-inline" method="post">
						<input type="hidden" name="orderTime.id" id="timeid"/>
