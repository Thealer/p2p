			...
			<div class="col-sm-9">
				...
				<form id="searchForm" class="form-inline" method="post" action="/realAuth.do">
					...
					<div class="form-group">
					    <label>状态</label>
					    <select class="form-control" name="state" id="tempState">
					    	<option value="-1">全部${qo.state}</option>
					    	<option value="0">申请中</option>
					    	<option value="1">审核通过</option>
					    	<option value="2">审核拒绝</option>
					    </select>
						<script type="text/javascript">
							$("#tempState option[value=${qo.state}]").attr("selected",true);
						</script>
					</div>
					<div class="form-group">
					    <label>申请时间</label>
					    <input class="form-control" type="text" name="beginDate" id="beginDate" value="" />到
